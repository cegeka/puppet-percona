Puppet::Type.type(:percona_user).provide(:mysql) do
  ...
  def create
    mysql([defaults_file, "mysql", "-e", "create user '%s' identified with sha256_password as '%s'" % [ @resource[:name].sub("@", "'@'"), @resource.value(:password_hash) ]].compact)
  end
  ...
  def password_hash=(string)
    mysql([defaults_file, "mysql", "-e", "ALTER user '%s' identified with sha256_password as '%s'" % [ @resource[:name].sub("@", "'@'"), string ] ].compact)
  end
